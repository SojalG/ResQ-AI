...
app.post("/api/resources/suggest", async (req, res, next) => {
  try {
    const { zone, floodRisk } = req.body;
    const recommendation = await getRecommendation({ zone, floodRisk });
    const resources =
      floodRisk >= 70
    ...
    res.json({ zone, resources, recommendation });
  } catch (e) {
    next(e);
  }
});
...
app.listen(port, () =>
  console.log(`ResQAI API running on http://localhost:${port}`),
);
